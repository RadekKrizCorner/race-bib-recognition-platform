...
def main() -> None:
    """Start the linking worker."""
    logger = configure_logging("linking-service")
    ...
    processor = ReliableStageProcessor(
        stage="LINKING",
        service_name="linking-service",
        input_topic=EventType.BIB_OCR_COMPLETED.topic_name(),
        repository=repository,
        event_bus=event_bus,
    )

    def handler(event: EventEnvelope) -> EventEnvelope | None:
        """Process one linking event with reliability handling."""
        return processor.process(event, runner.linking_handler.handle)

    if process_stdin_event(handler) is None:
        bootstrap_servers = os.getenv("RBP_KAFKA_BOOTSTRAP_SERVERS")
        if bootstrap_servers:
            logger.info("Linking worker is consuming Kafka.")
            KafkaStageWorker(
                bootstrap_servers=bootstrap_servers,
                group_id="linking-service",
                input_topic=EventType.BIB_OCR_COMPLETED.topic_name(),
                handler=handler,
            ).run_forever()
        else:
            logger.info("Linking worker is waiting for Kafka integration.")
            idle_forever("linking-service")
