...
def main() -> None:
    ...
    logger = configure_logging("ocr-service")
    ...
    processor = ReliableStageProcessor(
        ...
        input_topic=EventType.BIB_NORMALIZED.topic_name(),
        repository=repository,
        event_bus=event_bus,
    )

    def handler(event: EventEnvelope) -> EventEnvelope | None:
        """Process one OCR event with reliability handling."""
        return processor.process(event, runner.ocr_handler.handle)

    if process_stdin_event(handler) is None:
        bootstrap_servers = os.getenv("RBP_KAFKA_BOOTSTRAP_SERVERS")
        if bootstrap_servers:
            logger.info("OCR worker is consuming Kafka.")
            KafkaStageWorker(
            ...
            logger.info("OCR worker is waiting for Kafka integration.")
            idle_forever("ocr-service")
